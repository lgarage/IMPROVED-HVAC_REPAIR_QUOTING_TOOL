...
// ====================================================================
// --- FIELD-PIPELINE DRAFT QUOTE CREATION (Slice 64i) ---
// ====================================================================
...
function createOfficeDraftQuote(quoteData, ticketId, ticketDoc) {
    ...
    qRef.where("ticketId", "==", ticketId).where("status", "==", "Draft").limit(1).get()
        ...
        .catch(function (err) {
            console.warn("[QuotePipeline] Error checking existing drafts:", err);
            alert("Could not check for existing drafts. See console for details.");
        });
}

function _doCreateDraftQuote(quoteData, ticketId, ticketDoc, qRef) {
    ...
    var draft = {
        ...
        locationAddress: ticketDoc.address || ticketDoc.serviceAddress || ticketDoc.locationAddress || "",
        status: "Draft",
        jobWorkflow: "N/A",
