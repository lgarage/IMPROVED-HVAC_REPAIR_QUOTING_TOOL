...
- **Cross-device override flag (`officeOverrideActive`):** `service_call.js#toggleOfficeOverride` writes **`{ officeOverrideActive, officeOverrideBy, officeOverrideAt }`** to the open ticket via **`VCFirestore.setServiceCallMerged`**, and clears those fields (with **`FieldValue.delete()`**) when the dispatcher toggles off / closes the preview / unloads the tab. **`technician/js/workspace_ui.js#subscribeOfficeOverrideForTicket(ticketId)`** opens an `onSnapshot` on the active ticket from `workspaceUiOnOpen`, calls **`handleOfficeOverride(active)`**, and updates the strip label with **`officeOverrideBy`**. `switchScreen('!workspace')` calls **`teardownOfficeOverrideTicketListener()`**.
