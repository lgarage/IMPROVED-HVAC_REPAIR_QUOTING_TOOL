...
## Feature Index (grep first, then jump to section)

| Feature / Area | Phase | Key file(s) | Section |
|----------------|-------|-------------|---------|
| Tenant isolation + bridge (TWIN_PILLARS) | 10–11 | `shared/firebase_logic.js` | §1 |
| Sandbox / training accounts | — | `technician/js/vc_entitlements.js` | §1 |
| Dispatcher navigation + sidebar | — | `index.html`, `dispatcher/css/sidebar.css` | §2.1 |
| Service Call Intake + dispatch board | — | `service_call.js`, `dispatcher/js/ticket_manager.js` | §2.2 |
| Tech availability + fleet capacity | 23–24 | `ticket_manager.js`, `insights_manager.js` | §2.2 |
| Quoting Tool | — | `quoting.js` | §2.3 |
| Invoicing | — | `invoice.js` | §2.4 |
| Customer Directory + Site History | — | `customer_directory.js` | §2.5 |
| Inter-Office Feed (Pulse) — gated `vcHasFeature` | 35a | `index.html` | §2.6 |
| Executive Insights & Revenue Dashboard | 15 | `dispatcher/js/insights_manager.js` | §2.7 |
| Custom Report Studio | 16 | `dispatcher/js/report_builder.js` | §2.8 |
| AI Report Reviewer | 29 | `dispatcher/js/ai_report_reviewer.js` | §2.9 |
| Shadow Mode + Watch + Take Over | 19–31 | `dispatcher/js/shadow_mode.js` | §2.10 |
| Office Override (iframe edit + consent) | 30–32 | `service_call.js`, `technician/js/workspace_ui.js` | §2.11 |
| Settings — Members, Admins, Entitlements, Per-User | 35–36 | `index.html`, `shared/auth.js` | §2.12 |
...
- API: `generativelanguage.googleapis.com` `generateContent`, model from `GEMINI_GENERATE_MODEL` or default `gemini-2.0-flash`.
...
> **Moved to cold archive.** Full per-phase shipping inventory with commit-level detail is in `PROJECT_MAP_HISTORY.md`. Do not read that file proactively — grep by phase number or feature name only when you need to verify what shipped.

### Phase summary (quick reference)

| Phase | What shipped | Status |
|-------|-------------|--------|
| 10–11 | Tenant isolation, data bridge, terminology pivot | ✅ |
| 12–18 | Enterprise import, Lite seat, Proof of Service, Insights, Evidence, Charts, Offline sync | ✅ |
| 19–20 | Shadow Mode + force sync | ✅ |
| 23–24 | Tech availability, fleet capacity, conflict detection | ✅ |
| 27–29 | Intelligent workspace, AI Report Reviewer, Dispatch QoL | ✅ |
| 30–32c | Office Override (iframe + consent gate + KI-001 fix) + Watch+TakeOver + KI-002 sync hardening | ✅ |
| 33 | Field-Add Equipment — single canonical store, bridge helpers, per-field provenance (ADR-011) | ✅ |
| 34a–34e | Field Form Builder + 9 seed templates + repair branching + thermostat labeling + Site Intel Access Photos | ✅ |
...
_Detailed per-phase shipping inventory with commit-level detail: see `PROJECT_MAP_HISTORY.md`._

- [v] Phase 10–11: Tenant isolation, data bridge, terminology pivot
- [v] Phase 12–18: Enterprise import, Lite seat, Proof of Service, Insights, Evidence, Charts, Offline sync
- [v] Phase 19–20: Shadow Mode + force sync
- [v] Phase 23–24: Tech availability, fleet capacity, conflict detection
- [v] Phase 27–29: Intelligent workspace, AI Report Reviewer, Dispatch QoL
- [v] Phase 30–32c: Office Override (iframe + consent gate + KI-001 fix) + Watch+TakeOver + KI-002 sync hardening — see `DECISIONS.md → ADR-008/ADR-010` + `KNOWN_ISSUES_ARCHIVE.md → KI-002`
- [v] Phase 34a–34e: Field Form Builder + 9 seed templates + repair branching accordion + thermostat labeling + Site Intel Access Photos — see `DECISIONS.md → ADR-014` + `PROJECT_MAP_HISTORY.md`
