...
- `dictation_hub.js`: `SYSTEM_INSTRUCTION` merges **improvedNotes** rules with Rosetta; `processVisitNotes` writes `improvedNotes` back into `#dictationHubNotes` when present; **➕ Add Equipment** (`#btnAddEquipmentPill`, below **View Site Equipment**) opens Vision Hub (no manual prompt-only flow).

#### Vision Hub — full-screen Add Equipment (Phase 27)

**User Guide**

- Tap **➕ Add Equipment** (pill below **🗄️ View Site Equipment**) to open **Vision Hub** (full-screen overlay). **Capture nameplate photo** runs **Gemini Vision** preview (`dictationPreviewNameplateFromFile`) to fill manufacturer, model, serial, and **capacity (BTU / tons)**; edit **Unit type** + **Unit #**, then **Save equipment** to merge the Firestore asset and upload the nameplate via `dictationPromoteAssetPhoto`. On phones, **Capture nameplate photo** uses the **native picker** (Take Photo / Photo Library / Choose File) — not rear-camera-only.
...
- **Technician notes** (header on the workspace) is the primary notes area: **✨ Improve with AI** and a single textarea. **View Site Equipment** + **➕ Add Equipment** sit **below** the notes area; the **Dynamic action tray — units on site** section is **hidden in the UI** (2026-05-02) — unit tiles still load in the hidden `#dictationActionTray` in the background so `#linkedEquipmentSelect`, Phase 34d thermostat labeling (`getDerivedTstatUnitLabel`), and Rosetta overlays keep working. Day-to-day equipment discovery is via **Equipment Hub** and **Vision Hub**. There is **no** public vs. internal toggle — techs use one unified notes box; content is synced to the office for Inter-Office / dispatcher workflows (including **AI Report Reviewer** on raw `internal_comms`). The office formats customer-facing copy on the dispatcher side.

**Technical Specs**

- `technician/index.html`: `.dictation-action-tray-wrap { display: none !important; }` — hides the tray **visually only**; `#dictationActionTray` stays in the DOM so `dictation_hub.js#renderActionTray`, `getDerivedTstatUnitLabel()` (first non-ghost `.dictation-asset-card-id`), and the thermostat `MutationObserver` on the tray remain valid.
...
#### Site Intelligence — Field Access Notes (Phase 34e)

**User Guide**

- **Site Intel** button (`#wsSiteIntelBtn`) opens a modal for **persistent site notes** (ladder access, roof hatch, lock codes) shared across all jobs at that **location line**. The notes field is labeled **Field Access Notes** (renamed from “Field Bible”).
- **Access Photos** in the same modal lets techs attach pictures of access issues or processes. When notes or photos exist, the button gets a visual **has data** state.
...
- Below the notes textarea, an **Access Photos** section lets the technician capture and store photos of access issues or processes for future technicians (ladder placement, key locations, hatch routes, etc.). Tap **📷 Add photo** → the device shows the **native picker** (**Take Photo / Photo Library / Choose File** on phones). Each uploaded photo shows as a thumbnail with an inline caption box and a red **×** delete button.
...
- **Post-ship fix (2026-05-02) — Action Tray "+ Add Equipment" pill + Vision Hub native picker:** Pulled the **"+ Add Equipment"** trigger out of the Action Tray cards row and turned it into a standalone pill button (`#btnAddEquipmentPill` — full-width, rounded `999px`, cyan `#0ea5e9` outline) placed directly below the **🗄️ View Site Equipment** button in `technician/index.html`. The dynamic action tray now stays focused on existing units only. `dictation_hub.js?v=11`: `buildAddEquipmentCardHtml()` returns empty string (kept callable for cached-HTML fallback safety); empty-state copy updated to point at the new pill ("Tap **➕ Add Equipment** below the 'View Site Equipment' button"); `wireAddEquipmentPillButton()` (idempotent, `dataset.vcWired` guard) wired from `startDictationHubFromWorkspace` to call `openVisionHubAddEquipment` on click; existing `.dictation-add-equipment-btn` tray-delegation handler retained as defensive fallback for any cached HTML still rendering the old card. Also removed `capture="environment"` from `#visionHubCameraInput` so the Vision Hub nameplate capture shows the native picker sheet (**Take Photo / Photo Library / Choose File**) — a tech can now pull a photo from their library when on-site capture isn't possible. Cache-bust: `dictation_hub.js?v=10` → `?v=11`, `window.VC_BUILD = "Phase34e-2026-05-02"` → `"Phase34e-pill-2026-05-02"` in `technician/index.html`. Dispatcher (`index.html`) NOT touched.
- **Post-ship fix (2026-05-02) — Hide Dynamic action tray UI:** `.dictation-action-tray-wrap { display: none !important; }` in `technician/index.html` so the **"Dynamic action tray — units on site"** heading + strip no longer appears under the notes textarea. **`#dictationActionTray` remains in the DOM** so `renderActionTray`, `getDerivedTstatUnitLabel` (thermostat label fallback), and the tray `MutationObserver` still run; users rely on **View Site Equipment** + **➕ Add Equipment** for equipment UX. Rides the existing `VC_BUILD` stamp (CSS-only change).
...
- [v] Phase 35 — **Customer Entitlements & Feature-Flag Platform.** New `shared/entitlements.js` (`VCEntitlements` module): plan + feature catalog (`FEATURE_CATALOG` — `interOfficeFeed`, `aiReportReviewer`, `executiveInsights`, `customReportStudio`, `fieldAppOfficeOverride`, `shadowMode`, `siteHistory`, `quotingTool`, `invoicing`, `customerDirectory`); default-deny `has(featureId)`; `subscribe`/`savePatch`/`applyPlanDefaults`; `localStorage` cold-boot cache (`vc_entitlements_cache_v1`); `vc:entitlements-changed` event. Backed by `tenants/{tenantId}/config/entitlements`. New dispatcher Settings admin section "🎚️ Plan & Feature Entitlements" (plan select Free/Pro/Enterprise/Custom, per-feature toggles, Apply-plan-defaults + Save buttons). Wired into `index.html` boot (subscribe-on-Firebase-ready) and `technician/index.html` (include line so field app sees same flags). `vcHasFeature` global alias for `VCEntitlements.has`. **Phase 35a** — Inter-Office Feed (Pulse) re-introduced, gated behind `vcHasFeature("interOfficeFeed")`: restored CSS/JS includes, sidebar `#sidebar-nav-pulse` (hidden by default, shown/hidden by `syncPulseNav` on boot + `vc:entitlements-changed`), `#view-pulse` dashboard + quick-reply, `switchTab` deactivation, Pulse start/stop guard, `#pulse` hash routing, `wireQuickReplyOnce`; live nav sync on settings save (no reload). Files: `shared/entitlements.js` (NEW), `index.html` (boot wiring + Settings UI + `VC_BUILD`), `technician/index.html` (include). `VC_BUILD = "Gated-InterOffice-Feed-2026-05-06"`. ADR logged in dossier (Plan & Feature Entitlements row, 2026-05-06).
- [ ] Phase 36 — **Per-User Feature Toggles** (Slice 1 of 5 shipped). **Slice 1 (shipped 2026-05-06):** `firestore.rules` (NEW — default-deny by enumeration; locks `tenants/{tid}/config/entitlements` + `tenants/{tid}/admins/*` + `users/{uid}.featureOverrides` field writes to admin; preserves open behavior on all legacy paths); `firestore.indexes.json` (NEW); `firebase.json` gains `firestore` block; `shared/auth.js` (NEW — `VCAuth` module: `onReady`/`signIn`/`signOut`/`isAdmin`/`requireAdmin`/`isPinUnlocked`; resolves admin via bootstrap email OR `tenants/{tid}/admins/by_uid` doc OR roster `isAdmin === true`); `APP_CONFIG.bootstrapAdminEmails: []` on `shared/config.js`; `index.html` loads `firebase-auth-compat.js@10.8.1` + `shared/auth.js?v=1`; `shared/config.js?v=6→v=7`; `VC_BUILD = "Slice1-Auth-Foundation-2026-05-06"`. **Rules NOT deployed yet** — user must edit bootstrap emails in both `shared/config.js` and `firestore.rules`, then `firebase deploy --only firestore:rules`. Slices 2–5 tracked in `PROJECT_STATUS/PER_USER_FEATURE_TOGGLES_PLAN.md`. See `DECISIONS.md → ADR-015`.
