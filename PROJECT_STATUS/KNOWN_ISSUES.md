...
- **Status (2026-05-20 — HOOKS SHIPPED):** `VCStorageOutbox.registerHook()` + `hookContext` 4th param added to `enqueue()`. `drain()` now calls `ref.getDownloadURL()` after put() and invokes the registered hook with `(url, payload)` so callers can patch Firestore. Hook wiring shipped for 7 call sites: `dictation_hub.js#visionHubNameplate`, `equipment_manager.js#equipmentOverallPhoto`, `equipment_manager.js#equipmentDataPlatePhoto`, `equipment_manager.js#dictationAssetPhoto`, `field_forms.js#fieldFormEquipmentPhoto`, `workspace_ui.js#siteIntelAccessPhoto`, `unit_work_parser.js` (reuses equipment hooks), `technician/index.html#fieldEvidencePastedPhoto`. `VC_BUILD: KI004-Hooks-2026-05-21`. `sw.js CACHE_NAME` → `vertex-cache-v4` (B6). Dispatcher-side uploads (`service_call.js` ×2) intentionally not wired per ADR-012. Skipped hooks (no stable doc target at enqueue time): `conversational_timeline.js`, `teaching_layer.js`, addendum photos (fileUrls loop), `field_forms.js` bulk-photo loops.
- **Remaining:** On-device smoke test — take photo offline, re-enable signal, verify Firestore doc URL field patches automatically without tech re-saving.
