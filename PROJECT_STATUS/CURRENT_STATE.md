...
- **Active Phase:** **Phase 32b — Root-cause diagnosis (no-ticket-loaded write skip + per-ticket field dump)** (shipped to repo 2026-04-25; **needs on-device verification**). 32a's always-on debug overlay revealed the actual root cause of "consent button doesn't appear on the iPhone": when the dispatcher clicks `#btnOfficeOverride` (top of the phone simulator preview) **without first loading a ticket in the Service Call Intake panel**, `#scCurrentId.value` is empty, the Firestore write is silently skipped, the iframe toggles locally only (postMessage path) — and the tech's real phone never sees a flag. 32b fixes the silent failure with a loud alert + red outline + console warn on the dispatcher side, and adds per-ticket field-level diagnostics to the debug overlay so you can see at a glance whether the dispatcher's write reached the doc. Phase 32 + 32a + 32b stack and ship together.
...
> **The #1 cause of "consent button doesn't show on the tech's phone": the dispatcher clicks `#btnOfficeOverride` (top of the phone simulator preview) WITHOUT first opening the ticket in the Service Call Intake panel.** When `#scCurrentId.value` is empty, `toggleOfficeOverride` silently skips the Firestore write — the iframe toggles locally but the tech's real phone never sees a flag. As of Phase 32b this triggers a red outline + alert on the dispatcher side. **Always open the tech's active ticket in Service Call Intake FIRST**, then click Office Override.

1. **Field-verify Phase 32 (consent gate) on a real iPhone.** On the dispatcher PC: **first open the tech's active ticket in the Service Call Intake panel** (so `#scCurrentId.value` is populated). Then click `#btnOfficeOverride` (top of the phone simulator preview) → it should flip to `Office Override (ACTIVE)` with no red outline / no alert. Expected on the tech's phone within ~1s:
