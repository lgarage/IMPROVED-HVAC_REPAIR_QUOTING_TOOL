...
- **Last shipped (2026-05-16):** **Build runner timestamps now use local Chicago time** (`tools/build_runner.ts`). Log lines, `lastRun`, and `lastAttempt` now format in America/Chicago instead of UTC so build history reads in CDT/CST. Previous: video upload error handling.
...
- **Timestamp fix:** `vertex` log entries and saved run markers now print in America/Chicago; use `/log` if you want to confirm the new local-time format.
