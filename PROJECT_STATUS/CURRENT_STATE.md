...
- **Active Phase:** **None — ready to pick the next phase.** Phases 31–32c verified end-to-end on a real iPhone 2026-04-25; **KI-002 Plan A** + **KI-002 Plan B subset (B1+B2+B3+B4)** both shipped 2026-04-25. Pick the next phase from **On Deck** below (recommend continuing KI-002 with Plan C3 next, then E2).
- **Last shipped this session (2026-04-25):** Phases 31 + 32 + 32a + 32b + 32c (Office Override consent gate end-to-end + on-device diagnosability) **+ KI-002 Plan A** (silent-failure repair: new `VCRequireTicketId` + `VCSurfaceWriteFailure` helpers in `shared/firebase_logic.js`, ring-buffered into `window.__vcWriteFailures` and surfaced live in the iPhone debug overlay; A1–A9 call sites converted; `service_call.js?v=68`; `VC_BUILD = "KI002-A-2026-04-25"`) **+ KI-002 Plan B subset B1+B2+B3+B4** (cache & version hygiene: `shared/firebase_logic.js?v=1` on all 3 callers + load-time `[VC] firebase_logic v=1 loaded` console.info + `window.__VC_FIREBASE_LOGIC_VERSION` global; `?v=1` on the 5 unversioned tech bundle scripts; `equipment_manager.js?v=8` unified across dispatcher + tech; new dispatcher `window.VC_BUILD = "KI002-B-2026-04-25"` + `#vcBuildChip` in sidebar-footer that renders `BUILD <stamp> · fb v<N>` and is click-to-copy). See `PROJECT_MAP.md → Build History` and `KNOWN_ISSUES.md → KI-002` for the full inventory and flipped checklists.
...
KI-002 Plan A and Plan B subset (B1+B2+B3+B4) are both shipped this session. KI-002 itself is still **Open** because C and E remain. Order to ship next:

1. **Plan C3** — fix the Shadow-mirror 40×350ms polling stall by falling back to `getServiceCallOnceBridged(tid)` after exhaustion. ~30 min.
2. **Plan E2** — TWIN_PILLARS branding purge across the codebase (start with `client_notifications.js#getTenantIdSafe`, then grep). ~30 min.
3. Verify on iPhone + dispatcher using the always-on debug overlay (surfaces `__vcWriteFailures` ring) + dispatcher BUILD chip (renders `BUILD <stamp> · fb v<N>`). Mark the relevant KI-002 checklist items, flip KI-002 itself once all of B/C/E that we care about for this pass is done.
4. **Then Phase 33 — Field-Add Equipment** (see `ROADMAP.md → Next Up`). Lock the data-path decision (write to `imported_equipment` only vs. mirror to legacy `customers/.../assets`) before code. Hierarchy: parent company → customer name → customer location → unit number.

**Verification protocol for the Plan A + Plan B batches:**
- *Tech (force-reload field app on iPhone):* debug overlay top line should read `BUILD: KI002-A-2026-04-25`. Smoke-test the Plan A surfaces with connectivity killed → (a) edit dictation notes → expect inline `⚠ note not synced — tap to retry` banner under `#dictationHubNotes`; (b) navigate screens → expect top-right `⚠ presence offline` chip after retry exhausts; (c) attempt a report upload → expect red `⚠ Sync Failed` card on `#successCard`. Restore connectivity → all three should clear.
- *Dispatcher (force-reload):* sidebar-footer chip should read `BUILD KI002-B-2026-04-25 · fb v1` (the `fb v1` half confirms `shared/firebase_logic.js?v=1` actually loaded). Open DevTools console → expect `[VC] firebase_logic v=1 loaded`. Click chip → "Copied" toast for 1.2s. Toggle Office Override with no ticket loaded → still triggers the Phase 32b empty-tid alarm; toggle with a real ticket but no connectivity → triggers the new Plan A4 write-failure alarm.
