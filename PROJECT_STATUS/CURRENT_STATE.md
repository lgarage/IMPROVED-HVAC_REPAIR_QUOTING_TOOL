...
- **Last shipped (2026-05-09):** Storage rules completed — added missing `field_evidence/` + `service_call_addendums/` prefixes (found in `technician/index.html`; prior grep was `*.js`-only); 10 explicit path blocks total, default deny; updated header comment.
- **Prior (2026-05-09):** Storage rules scoped — replaced open catch-all in `storage.rules` with 8 explicit path-prefix blocks (equipment_photos, dictation_hub_assets, customer_evidence, field_quote_evidence, field_form_evidence, quote_evidence, site_access_photos, tenants/imported_equipment_photos); default deny all other paths; mirrors `firestore.rules` enumeration strategy; `VC_BUILD = "StorageRulesScope-2026-05-09"`. **Still requires Firebase Console → Storage → Get Started + `firebase deploy --only storage`.**
