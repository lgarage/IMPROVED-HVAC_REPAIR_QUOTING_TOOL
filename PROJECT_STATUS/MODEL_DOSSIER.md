...
|------|----------------|-------|------|-----------|--------------|--------------|----------|---------|------|
| 2026-05-08 | Cloud Function OCR auth fix — `callGeminiVision` v2 onCall deployed behind Cloud Run with default "require auth"; field app has no Firebase Auth so every call rejected with 403 "not authenticated" before function code ran; fix: add `invoker: "public"` to `onCall` options; redeployed | LOW | T1 | Balanced | 90% | 95% | ok | ok | Cursor: **Sonnet 4.6**; `functions/index.js` (1-line option add) + redeploy. Lesson: v2 `onCall` deploys on Cloud Run which defaults to auth-required; `invoker: "public"` is mandatory for any callable function used by clients without Firebase Auth. Diagnosis from `firebase functions:log` — 3× "not authenticated" warnings were the direct root cause. |
