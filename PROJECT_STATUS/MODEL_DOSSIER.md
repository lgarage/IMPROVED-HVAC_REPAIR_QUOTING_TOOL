...
|------|----------------|-------|------|-----------|--------------|--------------|----------|---------|------|
| 2026-05-02 | Field app + simulator: Show previously submitted notes (`internal_comms` / `techNotes`) when tech opens a synced ticket on a fresh device — seed `#dictationHubNotes` from `activeTicket` in-memory + don't-clobber-with-empty guards on cloud fetch + live `onSnapshot` | UNCERTAIN→LOW | T2 | Strong | 72% | 88% | ok | ok | Cursor: **Opus 4.7**; `dictation_hub.js?v=14→15` `loadNotesFromStorageForNewTicket` falls back to `activeTicket.internal_comms` then `activeTicket.techNotes` when localStorage is empty; `fetchInternalCommsFromCloud` + `subscribeInternalCommsForTicket` add `if (!t && el.value.trim()) return;` to keep seeded prior-tech notes from being blanked by transient empty snapshots; `window.VC_BUILD = "Phase34k-prior-notes-hydrate-2026-05-02"`. Same change covers real phone + dispatcher in-portal simulator (both load `technician/index.html`). No tenant/Firestore-write changes. |
