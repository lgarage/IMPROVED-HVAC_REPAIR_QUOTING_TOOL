...
|------|----------------|-------|------|-----------|--------------|--------------|----------|---------|------|
| 2026-05-02 | Field app + simulator: Show previously submitted notes (`internal_comms` / `techNotes`) when tech opens a synced ticket on a fresh device — seed `#dictationHubNotes` from `activeTicket` in-memory **and** from a bridged cloud refetch with `techNotes` fallback; debug overlay surfaces `actT.ic.len` / `actT.tn.len` / `dh.notes.len` for triage | UNCERTAIN→LOW | T2 | Strong | 72% | 78% | needed_bigger | partial | Cursor: **Opus 4.7**; pass 1 `dictation_hub.js?v=14→15` `loadNotesFromStorageForNewTicket` in-memory fallback + don't-clobber-empty guards on `fetchInternalCommsFromCloud` + live `onSnapshot`; pass 2 `?v=15→16` extends `fetchInternalCommsFromCloud` to fall back to `data.techNotes` (bridge-aware) so the dispatcher's "Technician report (Field app):" panel content surfaces in the dictation hub even when `myTickets` excludes the ticket (>30-day schedule window) or the in-memory snapshot is stale; `technician/index.html` `vcDebugOverlay` adds `actT.ic.len` / `actT.tn.len` / `dh.notes.len` lines so a missing-notes report can be triaged without remote DevTools; `window.VC_BUILD = "Phase34k-prior-notes-cloud-fallback-2026-05-02"`. Same change covers real phone + dispatcher in-portal simulator. **Lesson:** initial scope guess (in-memory only) was incomplete; user re-reported same symptom → had to add cloud-side fallback + iPhone-visible diagnostic in a second pass. Future "tech notes don't appear" tasks: assume tier T2 minimum (Strong / Opus 4.7) and add an iPhone-visible diagnostic in pass 1, not pass 2. No tenant/Firestore-write changes. |
