...
|------|----------------|-------|------|-----------|--------------|--------------|----------|---------|------|
| 2026-05-02 | Field app + simulator: Block sync when recommended repair has quoteParts but no quoteLabor (`validateRecommendedRepairLabor`) | LOW | T1–T2 | Balanced | 76% | 88% | ok | ok | Cursor: **Sonnet 4.6**; `technician/index.html` new `validateRecommendedRepairLabor()`; wired before `validateNeedsRepairQuote` on both `#generate` + `#reportPreviewSyncBtn` |
