...
|------|----------------|-------|------|-----------|--------------|--------------|----------|---------|------|
| 2026-05-15 | Field app: Mobile UI v1 — remove FIELD APP header logo; Schedule button moved inline beside tech-name badge; Reason for Call hidden by default, tap ticket chip (SC-1002) to toggle; `toggleWorkspaceReasonBanner()` added; reset on ticket open; `VC_BUILD: Phase60-MobileUIv1-2026-05-15`. | LOW | T2 | Balanced | 85% | 90% | ok | ok | Cursor: **Sonnet 4.6** (model-switched). Pure HTML/CSS/JS chrome change — no Firestore writes, no auth paths. 6 targeted StrReplace ops in one file. Lesson: field app workspace chrome restructure (header layout, conditional panel) = T2/Balanced confirmed; Composer 2 could handle mechanical parts but Sonnet 4.6 appropriate for reasoning about CSS specificity + JS toggle state. |
