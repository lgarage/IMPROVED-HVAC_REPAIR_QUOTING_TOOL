...
|------|----------------|-------|------|-----------|--------------|--------------|----------|---------|------|
| 2026-05-08 | Shadow Mode consent gate — tech consent toggle in Profile panel writes `shadowConsent` to `live_presence`; dispatcher `shadow_mode.js` gates iframe on consent; consent-blocked overlay; real-time revoke/grant; persistent field app banner; no Firestore rules changes needed | HIGH | T3 | Strong | 76% | 88% | ok | ok | Cursor: **Opus 4.6**; files: `technician/index.html` (CSS + HTML + JS consent toggle/banner/heartbeat), `dispatcher/js/shadow_mode.js?v=6` (consent gate, loadShadowIframe, updateConsentGate), `index.html` (consent-blocked overlay HTML + CSS, cache-bust, VC_BUILD). Phase 37 — new Firestore field (`shadowConsent` on `live_presence`) + cross-surface coordination (field → Firestore → dispatcher real-time). Lesson: T3 Vertex Core / Opus 4.6 correct — Shadow Mode / Office Override domain, Firestore writes, multi-file, cross-surface real-time gating; no struggle, clean implementation. |
