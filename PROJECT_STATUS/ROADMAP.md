...
* **Phase 33 — Field-Add Equipment (No-Info Capture).** Spec locked 2026-04-25 in `DECISIONS.md → ADR-011`. Summary:
  * **Storage:** single canonical store `tenants/{tenantId}/imported_equipment` (CSV import + field-add + field-edit all write here).
  * **Existing readers:** new `getEquipmentForSiteBridged(db, customerId, siteId)` helper in `shared/firebase_logic.js` merges `imported_equipment` + legacy `customers/.../sites/.../assets` at read time (mirrors the ADR-003 bridge pattern). Equipment Hub, Custom Report Studio, and workspace equipment helpers migrate to this helper. Touching `shared/firebase_logic.js` triggers the KI-002 B1 lockstep `FIREBASE_LOGIC_VERSION` + `?v=N` bump on all 3 callers.
  * **Conflict policy:** **per-field truth** via `fieldEdits: { <fieldName>: { by, at } }` provenance map. CSV re-import skips any field present in `fieldEdits`; first-touch fields update freely. Field-as-truth always.
  * **Unit identity:** `{customerId}/{siteId}/{unitType}{unitNumber}` (e.g. `RTU3`, `B1`, `CU1`). Hybrid prefix list — canonical seed in `shared/config.js#VC_EQUIPMENT_TYPE_PREFIXES` (`RTU`, `B`, `CU`, `AHU`, `FCU`, `WH`, `MAU`, `EF`, `CHL`, `CT`, `SPLIT`) with **Other** escape hatch for freeform `unitType`.
  * **Edit scope (explicit non-goals):** Phase 33 supports edit-existing-slot + add-new-slot only. **No** relocate / retire / merge / delete in this phase — those are deferred to a future Equipment Lifecycle phase.
  * **Failure surfacing:** all writes use KI-002 Plan A standard helpers (`VCRequireTicketId`, `VCSurfaceWriteFailure`) with user-visible affordances per the Plan A discipline.
  * **Build entry points:** `dictation_hub.js#visionHubSaveEquipment` (already wired) for adds; existing equipment-edit flow in workspace for edits; new `getEquipmentForSiteBridged` in `shared/firebase_logic.js`; new `VC_EQUIPMENT_TYPE_PREFIXES` constant in `shared/config.js`; CSV importer per-field merge guard (location TBC at spec time).
