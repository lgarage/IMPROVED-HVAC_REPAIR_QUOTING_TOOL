...
| **Toggle-Recording Voice Input** — Replace PTT with toggle button (press once to record, press again to stop). Animated sine wave while recording. Instant transcription on stop. | `fresh` | — | PTT was shipped (Media Action Sheet, 2026-05-15). Toggle-record + sine wave animation is a new UX direction not yet discussed in Cursor. |
| **Slack-style Chat UI** — Cleaner lines, more readable, less cramped chat interface. Reference: Slack's line spacing and text hierarchy. | `fresh` | — | ChatGPT-style redesign shipped (Mobile UI v1–v8, 2026-05-15). Slack-style direction is a new iteration not yet discussed. |
