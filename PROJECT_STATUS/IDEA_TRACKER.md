...
| Vendor Directory — pick & email for parts quotes | `parked` | **F4** — tab stays; see `ICEBOX_FUTURE_IMPROVEMENTS.md` § F4 |
...
| **Vendor Directory — pick & email for parts quotes (in-app)** — **Icebox F4.** Directory tab **stays** (Phase 64d CRUD). **User wants:** (1) pick one or more vendors from the list (by category if helpful), (2) compose and send a **parts quote request email inside Vertex** — no leaving the app to copy `orders@…` addresses, (3) goal = get distributor pricing/availability for job parts. **v1:** multi-select + Request quote + compose modal + Gmail/`mailto:` send. **v2:** pre-fill from ticket/quote parts list + AI draft + Gmail API (`ai_quote_pipeline_spec.md` Phase 5–6). Full spec: `ICEBOX_FUTURE_IMPROVEMENTS.md` § F4. | `parked` | 2026-05-21 | User request (Johnstone-style vendor card screenshot). Not deleting directory — only the pick-and-email workflow is deferred. |
